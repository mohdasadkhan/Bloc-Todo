...
class TasksBloc extends HydratedBloc<TasksEvent, TasksState> {
  ...
  void _onUpdateTask(UpdateTask event, Emitter<TasksState> emit) {
    ...
    List<Task> initialPendingTasks = state.pendingTasks;
    List<Task> initialCompletedTasks = state.completedTasks;
    task.isDone == false
        ? {
            initialPendingTasks = List.from(initialPendingTasks)..remove(task),
            initialCompletedTasks = List.from(initialCompletedTasks)
              ..insert(0, task.copyWith(isDone: true))
          }
        : {
            initialCompletedTasks = List.from(initialCompletedTasks)
              ..remove(task),
            initialPendingTasks = List.from(initialPendingTasks)
  ...
  void _onDeleteTask(DeleteTask event, Emitter<TasksState> emit) {
    final state = this.state;
    List<Task> removedTask = List.from(state.removedTasks)..remove(event.task);
    emit(TasksState(
